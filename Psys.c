...
char *filename;
{
  ...
  float xxx, yyy, vxx, vyy; //* H2O change input Planet Par
  real	roche=0, msr, spr; //*H2O calculate roche
  ...
  while (fgets(s, 510, input) != NULL) {
    ...
    if (isalpha(s[0])) {
      s1 = s + strlen(nm);
      sscanf(s1 + strspn(s1, "\t :=>_"), "%f %f %f %f %f %f %s %s", &xxx, &yyy, &vxx, &vyy, &mass, &accret, test1, test2);
      if ( CICPlanet ) {
	...
	sqrt( (1.0-ECCENTRICITY)/(1.0+ECCENTRICITY) );
	  ...
	  */
	  /* H2O Initial Planet Par Manually for Binary Satellites;*/
	  /* Modify 2016/02/19
	  if (!feeldis) { 
		  sys->x[i] = xxx;
		  sys->y[i] = yyy;
	  ...
	  sys->x[i] = xxx;
	  msr = 1.0 + MASSRATIO;
	  roche = sys->x[i] * pow((msr*sys->mass[i]/(3.0*MASSRATIO)), 1./3.);
	  spr = roche*HRATIO;
	  sys->y[i] = (real)spr*yyy/msr;
	  sys->vx[i] = (real)sqrt(G*mass*(1.0+(1.0/MASSRATIO))/spr)*vxx/msr;
	  sys->vy[i] = vyy;
	  sys->acc[i] = accret;
	  sys->FeelDisk[i] = feeldis;
	  sys->FeelOthers[i] = feelothers;
	  ...
	  /* H2O Manually input binary DONE */
      i++;
    }
  }
