     ...
     real dt;
{
  ...
  real theta, omegadiff, ac, dc;/*H2O cos term theta*/
  nr = Vrad->Nrad;
  ns = Vrad->Nsec;
  rho = Rho->Field;
  vrad = Vrad->Field;
...
#pragma omp for
	...
	/* H2O */
	theta = (((real)j)/((real)ns))*4.0*PI;
	omegadiff = pow(Rmed[i],-3/2) - pow(OMEGAFRAME,-3/2);
	//vthetaint[l] += dt*TORR*pow(Rmed[i],-6);
	ac = sin(INCLI*PI/180.0);
	dc = cos(INCLI*PI/180.0);
	//vthetaint[l] += dt*TORS*(dc+ac*cos(theta))*pow(Rmed[i],-6)*omegadiff;
	/* H2O */
      }
    }
  }
     ...
     real dt;
{
  ...
  real dv;
  //real rcc, tcc;		// corotation radius H2O
  //real kk, xx, str,str2,tau;	// coratation index
  nr = Rho->Nrad;
  ns = Rho->Nsec;
  rho = Rho->Field;
  vrad = VradInt->Field;
...
#pragma omp for
	/**H2O**/ //calculatio
	//rcc = 1.0/(pow(OMEGAFRAME,(2.0/3.0)));		// calculate coration radius
	//tcc = 2.0*PI/OMEGAFRAME;			// calculate coration period
	//kk  = ((real)nr)*(rcc-RMIN)/(RMAX-RMIN);	// calculate index
	//str = RSIGMA*(((real)nr)/(RMAX-RMIN)); 		// Gaussion remove
	//str2= 2*str*str;
	/**H2O**/
    for (i = 0; i < nr; i++) {
      dxtheta = 2.0*PI/(real)ns*Rmed[i];
	...
	/**H2O**/ // consider remove rho 
	//xx = -pow((((real)i) - kk),2.0)/str2;		// Gaussion remove
	//rho[l] = rho[l]*(1.0-BETA*exp(xx)*dt);		// Gaussion remove
	//tau = 1.0/(TNUM*tcc*(pow(((real)i)/kk,BETA)));	// remove timescale 
	//rho[l] = rho[l] - dt*tau*rho[l];		// remove
	/**H2O**/
      }
    }
...
#pragma omp for
    ...
    for (i = 1; i < nr; i++) { /* Trp defined from i=1 */
      viscosity = FViscosity (Rmed[i], Rho); /* Global_Bufarray holds cs */
      for (j = 0; j < ns; j++) {
	l = j+i*ns;
	if (viscosity != 0.0) {
     ...
     real deltaT;
{
  ...
  real *soundspeed;
  real dt11 = 1.0e20, dt22 = 1.0e20,dt33=1.0e20,dt44=1.0e20;
  ...
  for (i = One_or_active; i < Max_or_active; i++) {
    ...
    for (j = 0; j < ns; j++) {
      ...
      if ((1.0/invdt1) < dt11) dt11 = 1.0/invdt1;
      if ((1.0/invdt2) < dt22) dt22 = 1.0/invdt2;
      if ((1.0/invdt3) < dt33) dt33 = 1.0/invdt3;
      if ((1.0/invdt4) < dt44) dt44 = 1.0/invdt4;
