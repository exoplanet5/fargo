...
//**H2O** for binary accretion each
void AccreteOntoBinary (Rho, Vrad, Vtheta, dt, sys)
     real dt;
     PolarGrid *Rho, *Vrad, *Vtheta;
     PlanetarySystem *sys;
{
  ...
  real PxPlanet, PyPlanet, vrcell, vtcell, vxcell, vycell, xc, yc;
  real dPxPlanet, dPyPlanet, dMplanet;
  real massra, msr, spr; // H2O define a mass ration
  nr     = Rho->Nrad;
  ns     = Rho->Nsec;
  dens   = Rho->Field;
  abs    = CellAbscissa->Field;
  ...
  vrad   = Vrad->Field;
  vtheta = Vtheta->Field;
  msr	 = sys->mass[0]/sys->mass[1];
	// ** H2O calculate separation
  spr = sqrt(pow((sys->x[0]-sys->x[1]),2.0)+pow((sys->y[0]-sys->y[1]),2.0));
  for (k=0; k < sys->nb; k++) {
    if (sys->acc[k] > 1e-10) {
      dMplanet = dPxPlanet = dPyPlanet = 0.0;
      /* Hereafter : initialization of W. Kley's parameters */
...
//**H2O**
void ImprovingPlanetsMass (Rho, Vrad, Vtheta, dt, sys)
     real dt;
     PolarGrid *Rho, *Vrad, *Vtheta;
     PlanetarySystem *sys;
{
  ...
  vtheta = Vtheta->Field;
  for (k=0; k < sys->nb; k++) {
    if ((sys->mass[k] > 1.0e-10) && (sys->mass[k] < MASSPLANET)) {
      dMplanet = dPxPlanet = dPyPlanet = 0.0;
      facc = dt*(sys->mass[k]/(27.275));
      Xplanet = sys->x[k];
      Yplanet = sys->y[k];
      VXplanet = sys->vx[k];
      VYplanet = sys->vy[k];
